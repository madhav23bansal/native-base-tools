...
function checkBranch() {
  ...
  if (
    ["minor", "patch", "canary-patch", "canary-minor"].includes(currentBranch)
  ) {
    setDefaultBranch();
  } else {
    console.log(
      "\nRelease can only be created from minor, patch, canary-patch, canary-minor  branch.\nYour current branch is",
      currentBranch
    );
  }
}
...
function setDefaultBranch() {
  try {
    var result = execSync(
      "echo `git remote show origin | grep 'HEAD branch'` | cut -d \":\" -f2-"
    );
...
async function promptQuestions() {
  ...
  if (packageJsonVersion.includes("alpha")) {
    ...
    if (!choice) {
      if (type) {
        console.error(
          "Please pass correct type or choose from options given below"
        );
      }
    ...
    if (!choice2) {
      if (value) {
        console.error(
          "Please pass correct value or choose from options given below"
        );
      }
...
function parseVersion(choice, choice2) {
  if (choice == "exact") {
    if (
      packageJsonVersion.includes("alpha") ||
      packageJsonVersion.includes("rc")
    ) {
